...
class WishlistDetail(APIView):
    ...
    psermisson_classes = [IsAuthenticated]
    ...
    def get(self, request, pk):
        whishlist = self.get_object(pk, request.user)
        serializer = WishlistSerializer(whishlist, context={"request": request}, )
        return Response(serializer.data)
    ...
    def put(self, request, pk):
        whishlist = self.get_object(pk, request.user)
        serializer = WishlistSerializer(whishlist, data=request.data, partial=True)
        if serializer.is_valid():
